...
void setdelay(JE_byte delay) { target = (delay * 16) + SDL_GetTicks(); }

void setjasondelay(int delay) { target = SDL_GetTicks() + delay * jasondelay; }
...
void service_wait_delay(void) {
  while (SDL_GetTicks() < target) {
    SDL_Delay(SDL_GetTicks() - target > SDL_POLL_INTERVAL
                  ? SDL_POLL_INTERVAL
                  : SDL_GetTicks() - target);
...
void wait_delayorinput(JE_boolean keyboard, JE_boolean mouse,
  ...
  while (SDL_GetTicks() < target &&
         !((keyboard && keydown) || (mouse && mousedown) ||
           (joystick && joydown))) {
    SDL_Delay(SDL_GetTicks() - target > SDL_POLL_INTERVAL
                  ? SDL_POLL_INTERVAL
                  : SDL_GetTicks() - target);
...
void JE_loadSndFile(const char *effects_sndfile, const char *voices_sndfile) {
  ...
  for (y = 0; y < sndNum; y++) {
    ...
    templ = (sndPos[1][y + 1] - sndPos[1][y]) -
            100; /* SYN: I'm not entirely sure what's going on here. */
    if (templ < 1)
      templ = 1;
...
void JE_playSampleNum(JE_byte samplenum) {
  JE_multiSamplePlay(digiFx[samplenum - 1], fxSize[samplenum - 1], 0,
                     fxPlayVol);
}
...
void JE_setTimerInt(void) { jasondelay = 1000.0f / (1193180.0f / speed); }

void JE_resetTimerInt(void) { jasondelay = 1000.0f / (1193180.0f / 0x4300); }
