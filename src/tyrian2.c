...
inline static void blit_enemy(SDL_Surface *surface, unsigned int i,
  ...
  if (enemy[i].filter != 0)
    blit_sprite2_filter(surface, x, y, *enemy[i].sprite2s, index,
                        enemy[i].filter);
  else
...
void JE_drawEnemy(
    int enemyOffset) // actually does a whole lot more than just drawing
{
  player[0].x -= 25;
  ...
  for (int i = enemyOffset - 25; i < enemyOffset; i++) {
    ...
    enemy_still_exists:
      ...
      /*X bounce*/
      if (enemy[i].ex <= enemy[i].xminbounce ||
          enemy[i].ex >= enemy[i].xmaxbounce)
        enemy[i].exc = -enemy[i].exc;

      /*Y bounce*/
      if (enemy[i].ey <= enemy[i].yminbounce ||
          enemy[i].ey >= enemy[i].ymaxbounce)
      ...
      for (int j = 3; j > 0; j--) {
        if (enemy[i].freq[j - 1]) {
          ...
          if (--enemy[i].eshotwait[j - 1] == 0 && temp3) {
            ...
            if (galagaMode &&
                (enemy[i].eyc == 0 || (mt_rand() % 400) >= galagaShotFreq))
              goto draw_enemy_end;

            switch (temp3) {
            case 252: /* Savara Boss DualMissile */
              if (enemy[i].ey > 20) {
                JE_setupExplosion(tempX - 8 + tempMapXOfs,
                                  tempY - 20 - backMove * 8, -2, 6, false,
                                  false);
                JE_setupExplosion(tempX + 4 + tempMapXOfs,
                                  tempY - 20 - backMove * 8, -2, 6, false,
                                  false);
              }
              break;
            case 251:; /* Suck-O-Magnet */
              const int attractivity =
                  4 -
                  (abs(player[0].x - tempX) + abs(player[0].y - tempY)) / 100;
              player[0].x_velocity +=
                  (player[0].x > tempX) ? -attractivity : attractivity;
              break;
            case 253: /* Left ShortRange Magnet */
              if (abs(player[0].x + 25 - 14 - tempX) < 24 &&
                  abs(player[0].y - tempY) < 28) {
                player[0].x_velocity += 2;
              }
              if (twoPlayerMode && (abs(player[1].x - 14 - tempX) < 24 &&
                                    abs(player[1].y - tempY) < 28)) {
            ...
            case 255:                                 /* Magneto RePulse!! */
              if (difficultyLevel != DIFFICULTY_EASY) /*DIF*/
              {
                if (j == 3) {
                  ...
                  const int repulsivity = 4 - (abs(player[0].x - tempX) +
                                               abs(player[0].y - tempY)) /
                                                  20;
                  if (repulsivity > 0)
                    player[0].x_velocity +=
                        (player[0].x > tempX) ? repulsivity : -repulsivity;
                }
              }
              break;
            default:
              /*Rot*/
              for (int tempCount = weapons[temp3].multi; tempCount > 0;
                   tempCount--) {
                ...
                enemyShot[b].sx =
                    tempX + weapons[temp3].bx[tempPos] + tempMapXOfs;
                ...
                if (weapons[temp3].aim > 0) {
                  ...
                  const int longest_side =
                      MAX(abs(relative_x), abs(relative_y));
                  enemyShot[b].sxm =
                      roundf((float)relative_x / longest_side * aim);
                  enemyShot[b].sym =
                      roundf((float)relative_y / longest_side * aim);
                }
              }
              break;
            }
        ...
        if (--enemy[i].launchwait == 0) {
          ...
          if (b > 0) {
            ...
            if (e->launchtype > 0 && e->launchfreq == 0) {
              if (e->launchtype > 90) {
                e->ex += mt_rand() % ((e->launchtype - 90) * 4) -
                         (e->launchtype - 90) * 2;
              } else {
                ...
                e->exc =
                    roundf(((float)target_x / longest_side) * e->launchtype);
...
start_level:
  ...
  if (!play_demo) {
    if ((!all_players_dead() || normalBonusLevelCurrent || bonusLevelCurrent) &&
        !playerEndLevel) {
      mainLevel = nextLevel;
      JE_endLevelAni();
...
start_level_first:
  ...
  /* JE_setVol(tyrMusicVolume, fxPlayVol >> 2); NOTE: MXD killed this because it
   * was broken */
  ...
  if (!play_demo && record_demo) {
    ...
    } while (dir_file_exists(get_user_directory(),
                             tempStr)); // until file doesn't exist
...
level_loop:
  ...
  allPlayersGone = all_players_dead() &&
                   ((*player[0].lives == 1 && player[0].exploding_ticks == 0) ||
                    (!onePlayerAction && !twoPlayerMode)) &&
                   ((*player[1].lives == 1 && player[1].exploding_ticks == 0) ||
                    !twoPlayerMode);
  ...
  if (!endLevel) // draw HUD
  {
    ...
    if (textErase > 0 && --textErase == 0)
      blit_sprite(VGAScreenSeg, 16, 189, OPTION_SHAPES,
                  36); // in-game message area
    ...
    } else // not galagaMode
    {
      ...
      } else if (player[0].is_alive &&
                 player[0].shield < player[0].shield_max && power > shieldT) {
        if (--shieldWait == 0) {
          shieldWait = 15;
    ...
    for (uint i = 0; i < 2; ++i) {
      ...
      if (old_weapon_bar[i] != item_power) {
        ...
        int x = twoPlayerMode ? 286 : 289, y = (i == 0)
                                                   ? (twoPlayerMode ? 6 : 17)
                                                   : (twoPlayerMode ? 100 : 38);
    ...
    if (twoPlayerMode || onePlayerAction) {
      ...
      if (temp != lastPower) {
        if (temp > lastPower)
          fill_rectangle_xy(VGAScreenSeg, 269, 113 - 11 - temp, 276,
                            114 - 11 - lastPower, 113 + temp / 7);
        else
          fill_rectangle_xy(VGAScreenSeg, 269, 113 - 11 - lastPower, 276,
                            114 - 11 - temp, 0);
  ...
  if (anySmoothies)
    VGAScreen =
        VGAScreen2; // this makes things complicated, but we do it anyway :(
  ...
  for (int z = 0; z < MAX_PWEAPON; z++) {
    if (shotAvail[z] != 0) {
      ...
      if (!player_shot_move_and_draw(z, &is_special, &tempShotX, &tempShotY,
      ...
      for (b = 0; b < 100; b++) {
        if (enemyAvail[b] == 0) {
          ...
          if (z == MAX_PWEAPON - 1) {
            ...
            collided = abs(enemy[b].ex + enemy[b].mapoffset -
                           (player[0].x + 7)) < temp;
            temp2 = 9;
            chain = 0;
            damage = 10;
            ...
            collided =
                ((enemy[b].enemycycle == 0) &&
                 (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX - tempX2) <
                  (25 + tempX2)) &&
                 (abs(enemy[b].ey - tempShotY - 12 - tempY2) <
                  (29 + tempY2))) ||
                ((enemy[b].enemycycle > 0) &&
                 (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX - tempX2) <
                  (13 + tempX2)) &&
            ...
            collided =
                ((enemy[b].enemycycle == 0) &&
                 (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX) < 25) &&
                 (abs(enemy[b].ey - tempShotY - 12) < 29)) ||
                ((enemy[b].enemycycle > 0) &&
                 (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX) < 13) &&
          ...
          if (collided) {
            if (chain > 0) {
              ...
              b = player_shot_create(0, SHOT_MISC, tempShotX, tempShotY, mouseX,
                                     mouseY, chain, playerNum);
              shotAvail[z] = 0;
              goto draw_player_shot_loop_end;
            }
            ...
            if (enemy[b].armorleft < 255) {
              ...
              for (unsigned int e = 0; e < COUNTOF(enemy); e++) {
                if (enemy[e].linknum == temp && enemyAvail[e] != 1 &&
            ...
            if (armorleft > damage) {
              if (z != MAX_PWEAPON - 1) {
                if (enemy[b].armorleft != 255) {
                  ...
                  JE_doSP(tempShotX + 6, tempShotY + 6, damage / 2 + 3,
                          damage / 4 + 2, temp2);
                }
              }
                ...
                for (temp3 = 0; temp3 < 100; temp3++) {
                  if (enemyAvail[temp3] != 1) {
                    ...
                    if ((temp3 == b) ||
                      ...
                      if (enemy[temp3].armorleft >
                          (unsigned char)enemy[temp3].edlevel)
                      ...
                      if (enemyDat[enemy[temp3].enemytype].esize != 1)
                        ...
                        JE_setupExplosionLarge(enemy[temp3].enemyground,
                                               enemy[temp3].explonum / 2, tempX,
                                               tempY);
                    }
                  }
                }
                  ...
                  if ((temp2 == b) || (temp == 254) ||
                      ((temp != 255) &&
                       ((temp == temp3) || (temp - 100 == temp3) ||
                        ((temp3 > 40) && (temp3 / 20 == temp / 20) &&
                         (temp3 <= temp))))) {

                    int enemy_screen_x =
                        enemy[temp2].ex + enemy[temp2].mapoffset;

                    if (enemy[temp2].special) {
                      assert((unsigned int)enemy[temp2].flagnum - 1 <
                             COUNTOF(globalFlags));
                      globalFlags[enemy[temp2].flagnum - 1] =
                          enemy[temp2].setto;
                    }
                    ...
                    if ((enemy[temp2].evalue > 0) &&
                        (enemy[temp2].evalue < 10000)) {
                      if (enemy[temp2].evalue == 1) {
                        cubeMax++;
                      } else {
                        // in galaga mode player 2 is sidekick, so give cash to
                        // player 1
                        player[galagaMode ? 0 : playerNum - 1].cash +=
                            enemy[temp2].evalue;
                      }
                    }
                    ...
                    if (enemyDat[enemy[temp2].enemytype].esize == 1) {
                      JE_setupExplosionLarge(enemy[temp2].enemyground,
                                             ...
                                             enemy_screen_x, enemy[temp2].ey);
                      soundQueue[6] = S_EXPLOSION_9;
                    } else {
                      JE_setupExplosion(enemy_screen_x, enemy[temp2].ey, 0, 1,
                                        false, false);
  ...
  if (!endLevel) { /*MAIN DRAWING IS STOPPED STARTING HERE*/
    ...
    for (int z = 0; z < ENEMY_SHOT_MAX; z++) {
      if (enemyShotAvail[z] == 0) {
        ...
        if (enemyShot[z].duration-- == 0 || enemyShot[z].sy > 190 ||
            enemyShot[z].sy <= -14 || enemyShot[z].sx > 275 ||
            enemyShot[z].sx <= 0) {
        ...
        } else // check if shot collided with player
        {
          for (uint i = 0; i < (twoPlayerMode ? 2 : 1); ++i) {
            if (player[i].is_alive &&
                enemyShot[z].sx >
                    player[i].x - (signed)player[i].shot_hit_area_x &&
                enemyShot[z].sx <
                    player[i].x + (signed)player[i].shot_hit_area_x &&
                enemyShot[z].sy >
                    player[i].y - (signed)player[i].shot_hit_area_y &&
                enemyShot[z].sy <
                    player[i].y + (signed)player[i].shot_hit_area_y) {
    ...
    if (explosions[j].ttl != 0) {
      ...
      if (explosions[j].y > 200 - 14) {
        ...
        if (explosionTransparent)
          blit_sprite2_blend(VGAScreen, explosions[j].x, explosions[j].y,
                             explosionSpriteSheet, explosions[j].sprite + 1);
        else
          blit_sprite2(VGAScreen, explosions[j].x, explosions[j].y,
                       explosionSpriteSheet, explosions[j].sprite + 1);
  ...
  if ((player[0].is_alive && player[0].armor < 6) ||
      (twoPlayerMode && !galagaMode && player[1].is_alive &&
       player[1].armor < 6)) {
    int armor_amount = (player[0].is_alive && player[0].armor < 6)
                           ? player[0].armor
                           : player[1].armor;
    ...
    if ((player[0].is_alive && player[0].armor < 6 &&
         (!isNetworkGame || thisPlayerNum == 1)) ||
        (twoPlayerMode && player[1].is_alive && player[1].armor < 6 &&
         (!isNetworkGame || thisPlayerNum == 2))) {
  ...
  if (displayTime > 0) {
    displayTime--;
    JE_outTextAndDarken(VGAScreen, 90, 10, miscText[59], 15, (JE_byte)flash - 8,
                        FONT_SHAPES);
    flash += flashChange;
    if (flash > 4 || flash == 0)
  ...
  if (levelTimer && levelTimerCountdown > 0) {
    ...
    JE_textShade(VGAScreen, 140, 6, miscText[66], 7,
                 (levelTimerCountdown % 20) / 3, FULL_SHADE);
    ...
    if (allPlayersGone) {
      if (player[0].exploding_ticks > 0 || player[1].exploding_ticks > 0) {
        ...
        if (play_demo || normalBonusLevelCurrent || bonusLevelCurrent)
          ...
          JE_dString(VGAScreen, 120, 60, miscText[21],
                     FONT_SHAPES); // game over
  ...
  if (isNetworkGame) {
    if (!reallyEndLevel) {
      Uint16 requests =
          (pauseRequest == true) | (inGameMenuRequest == true) << 1 |
      ...
      if (network_state_update()) {
        ...
        if (requests & 2) {
          yourInGameMenuRequest =
              SDLNet_Read16(&packet_state_out[network_delay]->data[14]) & 2;
          JE_doInGameSetup();
          yourInGameMenuRequest = false;
          if (haltGame)
            reallyEndLevel = true;
        ...
        for (int i = 0; i < 2; i++) {
          if (SDLNet_Read16(&packet_state_in[0]->data[18 + i * 2]) !=
                  SDLNet_Read16(
                      &packet_state_out[network_delay]->data[18 + i * 2]) ||
              SDLNet_Read16(&packet_state_in[0]->data[20 + i * 2]) !=
                  SDLNet_Read16(
                      &packet_state_out[network_delay]->data[20 + i * 2])) {
            char temp[64];
      ...
      do {
        ...
        if (s[0] == ']') {
          ...
          case 'Q':
            ...
            if (twoPlayerMode) {
              for (uint i = 0; i < 2; ++i)
                snprintf(levelWarningText[i], sizeof(*levelWarningText),
                         "%s %lu", miscText[40], player[i].cash);
              strcpy(levelWarningText[2], "");
              levelWarningLines = 3;
            } else {
              sprintf(levelWarningText[0], "%s %lu", miscText[37],
            ...
            if (jumpBackToEpisode1 && !twoPlayerMode) {
              ...
              if (superTyrian) {
                // if completed Zinglon's Revenge, show SuperTyrian and Destruct
                // codes if completed SuperTyrian, show Nort-Ship Z code
                superArcadeMode =
                    (initialDifficulty == DIFFICULTY_ZINGLON) ? 8 : 1;
              }

              if (superArcadeMode < SA_ENGAGE) {
                if (SANextShip[superArcadeMode] == SA_ENGAGE) {
                  sprintf(buffer, "%s %s", miscTextB[4], pName[0]);
                  JE_dString(VGAScreen, JE_fontCenter(buffer, FONT_SHAPES), 100,
                             buffer, FONT_SHAPES);
                  ...
                  JE_dString(VGAScreen,
                             JE_fontCenter(superShips[0], FONT_SHAPES), 30,
                             superShips[0], FONT_SHAPES);
                  JE_dString(
                      VGAScreen,
                      JE_fontCenter(superShips[SANextShip[superArcadeMode]],
                                    SMALL_FONT_SHAPES),
                      100, superShips[SANextShip[superArcadeMode]],
                      SMALL_FONT_SHAPES);
                }

                if (SANextShip[superArcadeMode] < SA_NORTSHIPZ)
                  blit_sprite2x2(VGAScreen, 148, 70, spriteSheet9,
                                 ships[SAShip[SANextShip[superArcadeMode] - 1]]
                                     .shipgraphic);
                else if (SANextShip[superArcadeMode] == SA_NORTSHIPZ)
                  trentWin = true;

                sprintf(buffer, "Type %s at Title",
                        specialName[SANextShip[superArcadeMode] - 1]);
                JE_dString(VGAScreen, JE_fontCenter(buffer, SMALL_FONT_SHAPES),
                           160, buffer, SMALL_FONT_SHAPES);
                JE_showVGA();
          ...
          case 'P':
            if (!constantPlay) {
              tempX = atoi(s + 3);
              if (tempX > 900) {
                memcpy(colors, palettes[pcxpal[tempX - 1 - 900]],
                       sizeof(colors));
                JE_clr256(VGAScreen);
                JE_showVGA();
...
void networkStartScreen(void) {
  ...
  memcpy(VGAScreen2->pixels, VGAScreen->pixels,
         VGAScreen2->pitch * VGAScreen2->h);
  JE_dString(VGAScreen,
             JE_fontCenter("Waiting for other player.", SMALL_FONT_SHAPES), 140,
             "Waiting for other player.", SMALL_FONT_SHAPES);
  JE_showVGA();
  fade_palette(colors, 10, 0, 255);
  ...
  if (thisPlayerNum == 1) {
    ...
    JE_dString(VGAScreen, JE_fontCenter(networkText[4 - 1], SMALL_FONT_SHAPES),
               140, networkText[4 - 1], SMALL_FONT_SHAPES);
    JE_showVGA();
    ...
    while (true) {
      ...
      if (packet_in[0] &&
          SDLNet_Read16(&packet_in[0]->data[0]) == PACKET_DETAILS)
        break;
...
bool titleScreen(void) {
  ...
  SDL_strlcpy(menuText[4], opentyrian_str,
              sizeof menuText[4]); // OpenTyrian override
  ...
  for (;;) {
    if (restart) {
      ...
      draw_font_hv_shadow(VGAScreen, 2, 192, opentyrian_version, small_font,
                          left_aligned, 15, 0, false, 1);
      ...
      for (size_t i = 0; i < COUNTOF(menuText); ++i) {
        ...
        draw_font_hv(VGAScreen, x - 1, y - 1, menuText[i], normal_font,
                     left_aligned, 15, -10);
        draw_font_hv(VGAScreen, x + 1, y + 1, menuText[i], normal_font,
                     left_aligned, 15, -10);
        draw_font_hv(VGAScreen, x + 1, y - 1, menuText[i], normal_font,
        ...
        draw_font_hv(VGAScreen, x - 1, y + 1, menuText[i], normal_font,
                     left_aligned, 15, -10);
        draw_font_hv(VGAScreen, x, y, menuText[i], normal_font, left_aligned,
                     15, -3);
      }
    ...
    draw_font_hv(VGAScreen, VGAScreen->w / 2,
                 yMenuItems + hMenuItem * selectedIndex,
                 menuText[selectedIndex], normal_font, centered, 15, -1);
        ...
        if (mouse_x >= xMenuItem && mouse_x < xMenuItem + wMenuItem[i]) {
          ...
          if (mouse_y >= yMenuItem && mouse_y < yMenuItem + hMenuItem) {
            ...
            if (newmouse && lastmouse_but == SDL_BUTTON_LEFT &&
                lastmouse_x >= xMenuItem &&
                lastmouse_x < xMenuItem + wMenuItem[i] &&
    ...
    if (newmouse) {
      ...
      case SDL_SCANCODE_UP: {
        ...
        selectedIndex =
            selectedIndex == 0 ? COUNTOF(menuText) - 1 : selectedIndex - 1;
        break;
      }
      case SDL_SCANCODE_DOWN: {
        ...
        selectedIndex =
            selectedIndex == COUNTOF(menuText) - 1 ? 0 : selectedIndex + 1;
        break;
      }
...
bool newSuperArcadeGame(unsigned int i) {
  ...
  if (episodeSelect() && difficultySelect()) {
    ...
    JE_clr256(VGAScreen);
    JE_dString(VGAScreen, JE_fontCenter(superShips[0], FONT_SHAPES), 30,
               superShips[0], FONT_SHAPES);
    JE_dString(VGAScreen, JE_fontCenter(superShips[i + 1], SMALL_FONT_SHAPES),
               100, superShips[i + 1], SMALL_FONT_SHAPES);
...
void newSuperTyrianGame(void) {
  ...
  initialDifficulty = keysactive[SDL_SCANCODE_SCROLLLOCK] ? DIFFICULTY_SUICIDE
                                                          : DIFFICULTY_ZINGLON;

  JE_clr256(VGAScreen);
  JE_outText(VGAScreen, 10, 10, "Cheat codes have been disabled.", 15, 4);
  if (initialDifficulty == DIFFICULTY_ZINGLON)
    JE_outText(VGAScreen, 10, 20,
               "Difficulty level has been set to Lord of Game.", 15, 4);
  else
    JE_outText(VGAScreen, 10, 20, "Difficulty level has been set to Suicide.",
               15, 4);
  JE_outText(VGAScreen, 10, 30,
             "It is imperative that you discover the special codes.", 15, 4);
  if (initialDifficulty == DIFFICULTY_ZINGLON)
    JE_outText(VGAScreen, 10, 40,
...
void JE_displayText(void) {
  ...
  do {
    ...
    wait_delay();
      ...
      !(JE_anyButton() || (frameCountMax == 0 && temp == 1) || ESCPressed));
...
uint JE_makeEnemy(struct JE_SingleEnemyType *enemy, Uint16 eDatI,
                  Sint16 uniqueShapeTableI) {
  uint avail;
  ...
  if (sprite2s != NULL)
    ...
    // Use shape table value from previous enemy that occupied the enemy slot.
    // (Ex. APPROACH.)
  ...
  if (enemyDat[eDatI].startxc != 0)
    enemy->ex = enemyDat[eDatI].startx +
                (mt_rand() % (enemyDat[eDatI].startxc * 2)) -
                enemyDat[eDatI].startxc + 1;
  else
    enemy->ex = enemyDat[eDatI].startx + 1;

  if (enemyDat[eDatI].startyc != 0)
    enemy->ey = enemyDat[eDatI].starty +
                (mt_rand() % (enemyDat[eDatI].startyc * 2)) -
                enemyDat[eDatI].startyc + 1;
  else
...
void JE_createNewEventEnemy(JE_byte enemyTypeOfs, JE_word enemyOffset,
  ...
  if (eventRec[eventLoc - 1].eventdat2 != -99) {
    ...
    case 50:
      if (background3x1) {
        enemy[b - 1].ex =
            eventRec[eventLoc - 1].eventdat2 - (mapX - 1) * 24 - 12;
      } else {
        enemy[b - 1].ex =
            eventRec[eventLoc - 1].eventdat2 - mapX3 * 24 - 24 * 2 + 6;
      }
...
void JE_eventSystem(void) {
  ...
  case 5: // load enemy shape banks
  {
    const Uint8 newEnemyShapeTables[] = {
        eventRec[eventLoc - 1].eventdat > 0 ? eventRec[eventLoc - 1].eventdat
                                            : 0,
        eventRec[eventLoc - 1].eventdat2 > 0 ? eventRec[eventLoc - 1].eventdat2
                                             : 0,
        eventRec[eventLoc - 1].eventdat3 > 0 ? eventRec[eventLoc - 1].eventdat3
                                             : 0,
        eventRec[eventLoc - 1].eventdat4 > 0 ? eventRec[eventLoc - 1].eventdat4
                                             : 0,
    };
    ...
    for (unsigned int i = 0; i < COUNTOF(newEnemyShapeTables); ++i) {
      if (enemySpriteSheetIds[i] != newEnemyShapeTables[i]) {
        if (newEnemyShapeTables[i] > 0) {
          ...
          JE_loadCompShapes(&enemySpriteSheets[i],
                            shapeFile[newEnemyShapeTables[i] - 1]);
        } else
  ...
  case 12: /* Custom 4x4 Ground Enemy */
  {
    ...
    eventRec[eventLoc - 1].eventdat6 =
        0; /* We use EVENTDAT6 for the background */
  ...
  case 19: /* Enemy Global Move */
  {
    ...
    if (eventRec[eventLoc - 1].eventdat3 > 79 &&
        eventRec[eventLoc - 1].eventdat3 < 90) {
      initial_i = 0;
      max_i = 100;
      all_enemies = false;
      eventRec[eventLoc - 1].eventdat4 =
          newPL[eventRec[eventLoc - 1].eventdat3 - 80];
    } else {
  ...
  case 25: /* Enemy Global Damage change */
    for (temp = 0; temp < 100; temp++) {
      if (eventRec[eventLoc - 1].eventdat4 == 0 ||
          enemy[temp].linknum == eventRec[eventLoc - 1].eventdat4) {
        if (galagaMode)
          enemy[temp].armorleft =
              roundf(eventRec[eventLoc - 1].eventdat * (difficultyLevel / 2));
        else
    ...
    for (temp = 0; temp < 100; temp++) {
      if (eventRec[eventLoc - 1].eventdat4 == 0 ||
        ...
        if (eventRec[eventLoc - 1].eventdat3 != 0 &&
            eventRec[eventLoc - 1].eventdat3 < 17)
  ...
  case 33: /* Enemy From other Enemies */
    if (!((eventRec[eventLoc - 1].eventdat == 512 ||
           eventRec[eventLoc - 1].eventdat == 513) &&
      ...
      if (eventRec[eventLoc - 1].eventdat == 534 && superTyrian)
        eventRec[eventLoc - 1].eventdat =
            828 + superTyrianSpecials[mt_rand() % 4];
  ...
  case 46: // change difficulty
    ...
    if (eventRec[eventLoc - 1].eventdat2 == 0 || twoPlayerMode ||
        onePlayerAction) {
  ...
  case 61: // if specific flag set to specific value, skip events
    if (globalFlags[eventRec[eventLoc - 1].eventdat - 1] ==
  ...
  case 64:
    if (!(eventRec[eventLoc - 1].eventdat == 6 && twoPlayerMode &&
          difficultyLevel > DIFFICULTY_NORMAL)) {
      smoothies[eventRec[eventLoc - 1].eventdat - 1] =
          eventRec[eventLoc - 1].eventdat2;
  ...
  case 71:
    if (((((intptr_t)mapYPos - (intptr_t)&megaData1.mainmap) /
          sizeof(JE_byte *)) *
         2) <= (unsigned)eventRec[eventLoc - 1].eventdat2) {
      JE_eventJump(eventRec[eventLoc - 1].eventdat);
    }
    break;
  ...
  case 75:;
    ...
    if (temp_no_clue) {
      JE_byte enemy_i;
      do {
        temp = (mt_rand() % (eventRec[eventLoc - 1].eventdat2 + 1 -
                             eventRec[eventLoc - 1].eventdat)) +
               eventRec[eventLoc - 1].eventdat;
      ...
      if (eventRec[eventLoc - 1].eventdat4 > 0) { /*Skip*/
        curLoc = eventRec[eventLoc - 1 + eventRec[eventLoc - 1].eventdat4]
                     .eventtime -
                 1;
...
void JE_whoa(void) {
  ...
  memset((Uint8 *)VGAScreenSeg->pixels + screenSize - bottomBorder, 0,
         bottomBorder);

  /* Copy our test subject to one of the temporary buffers.  Blank the other */
  memset(TempScreen1, 0, screenSize);
  memcpy(TempScreen2, VGAScreenSeg->pixels,
         VGAScreenSeg->h * VGAScreenSeg->pitch);
  ...
  do {
    ...
    for (i = screenSize - bottomBorder, j = topBorder / 2; i > 0; i--, j++) {
      ...
      color = (TempScreen2[offset] * 12 +
               ...
               TempScreen1[offset - 1] + TempScreen1[offset + 1] +
               TempScreen1[offset + VGAScreenSeg->pitch]) /
              16;
    ...
    memcpy((Uint8 *)VGAScreenSeg->pixels + topBorder, TempScreen1 + topBorder,
           screenSize - bottomBorder);
...
static void JE_barX(JE_word x1, JE_word y1, JE_word x2, JE_word y2,
                    JE_byte col) {
...
void draw_boss_bar(void) {
  for (unsigned int b = 0; b < COUNTOF(boss_bar); b++) {
    ...
    if (armor > 255 || armor == 0) // boss dead?
      ...
      boss_bar[b].armor =
          (armor == 255) ? 254 : armor; // 255 would make the bar too long
  }

  unsigned int bars =
      (boss_bar[0].link_num != 0 ? 1 : 0) + (boss_bar[1].link_num != 0 ? 1 : 0);
  ...
  for (unsigned int b = 0; b < bars; b++) {
    unsigned int x =
        (bars == 2)
            ? ((b == 0) ? 125 : 185)
            : ((levelTimer) ? 250
                            : 155); // level timer and boss bar would overlap

    JE_barX(x - 25, 7, x + 25, 12, 115);
    JE_barX(x - (boss_bar[b].armor / 10), 7, x + (boss_bar[b].armor + 5) / 10,
            12, 118 + boss_bar[b].color);
