...
// 8-bit fwrite
static inline size_t fwrite_u8(const Uint8 *buffer, size_t count,
                               FILE *stream) {
  return fwrite(buffer, sizeof(Uint8), count, stream);
}

// 8-bit fwrite that dies if write fails
static inline void fwrite_u8_die(const Uint8 *buffer, size_t count,
                                 FILE *stream) {
  fwrite_die(buffer, sizeof(Uint8), count, stream);
}

// 8-bit fwrite that dies if write fails
static inline void fwrite_s8_die(const Sint8 *buffer, size_t count,
                                 FILE *stream) {
