...
static const char *getDisplayPickerItem(size_t i, char *buffer,
                                        size_t bufferSize) {
  if (i == 0)
    return "Window";
...
static size_t getScalerPickerItemsCount(void) { return (size_t)scalers_count; }

static const char *getScalerPickerItem(size_t i, char *buffer,
                                       size_t bufferSize) {
...
static const char *getScalingModePickerItem(size_t i, char *buffer,
                                            size_t bufferSize) {
  (void)buffer, (void)bufferSize;
...
void openTyrianMenu(void) {
  ...
  static const Menu menus[] = {
      [MENU_OPENTYRIAN] =
          {
              .header = "OpenTyrian",
              .items = {{MENU_ITEM_GRAPHICS, "Graphics...",
                         "Change the graphics settings."},
                        {MENU_ITEM_SOUND, "Sound...",
                         "Change the sound settings."},
                        {MENU_ITEM_JUKEBOX, "Jukebox",
                         "Listen to the music of Tyrian."},
                        // { MENU_ITEM_DESTRUCT, "Destruct", "Play a bonus
                        // mini-game." },
      ...
      [MENU_GRAPHICS] =
          {
              ...
              .items =
                  {{MENU_ITEM_DISPLAY, "Display:", "Change the display mode.",
                    getDisplayPickerItemsCount, getDisplayPickerItem},
                   {MENU_ITEM_SCALER,
                    "Scaler:", "Change the pixel art scaling algorithm.",
                    getScalerPickerItemsCount, getScalerPickerItem},
                   {MENU_ITEM_SCALING_MODE,
                    "Scaling Mode:", "Change the scaling mode.",
                    getScalingModePickerItemsCount, getScalingModePickerItem},
      ...
      [MENU_SOUND] =
          {
              .header = "Sound",
              .items = {{MENU_ITEM_MUSIC_VOLUME, "Music Volume",
                         "Change volume with the left/right arrow keys."},
                        {MENU_ITEM_EFFECTS_VOLUME, "Effects Volume",
                         "Change volume with the left/right arrow keys."},
                        {MENU_ITEM_DONE, "Done",
  ...
  for (;;) {
    ...
    // Draw header.
    draw_font_hv_shadow(VGAScreen, xCenter, yMenuHeader, menu->header,
                        large_font, centered, 15, -3, false, 2);
    ...
    for (size_t i = 0; menuItems[i].id != (MenuItemId)-1; ++i) {
      ...
      draw_font_hv_shadow(
          VGAScreen, xMenuItemName, y, name, normal_font, left_aligned, 15,
          -3 + (selected ? 2 : 0) + (disabled ? -4 : 0), false, 2);

      switch (menuItem->id) {
      case MENU_ITEM_DISPLAY:;
        const char *value = "Window";
        if (fullscreen_display >= 0) {
          snprintf(buffer, sizeof(buffer), "Display %d",
                   fullscreen_display + 1);
          value = buffer;
        }

        draw_font_hv_shadow(
            VGAScreen, xMenuItemValue, y, value, normal_font, left_aligned, 15,
            -3 + (selected ? 2 : 0) + (disabled ? -4 : 0), false, 2);
        break;

      case MENU_ITEM_SCALER:
        draw_font_hv_shadow(VGAScreen, xMenuItemValue, y, scalers[scaler].name,
                            normal_font, left_aligned, 15,
                            -3 + (selected ? 2 : 0) + (disabled ? -4 : 0),
                            false, 2);
        break;

      case MENU_ITEM_SCALING_MODE:
        draw_font_hv_shadow(
            VGAScreen, xMenuItemValue, y, scaling_mode_names[scaling_mode],
            normal_font, left_aligned, 15,
            -3 + (selected ? 2 : 0) + (disabled ? -4 : 0), false, 2);
        break;

      case MENU_ITEM_MUSIC_VOLUME:
        JE_barDrawShadow(VGAScreen, xMenuItemValue, y, 1,
                         music_disabled ? 170 : 174, (tyrMusicVolume + 4) / 8,
                         2, 10);
        JE_rectangle(VGAScreen, xMenuItemValue - 2, y - 2, xMenuItemValue + 96,
                     y + 11, 242);
        break;

      case MENU_ITEM_EFFECTS_VOLUME:
        JE_barDrawShadow(VGAScreen, xMenuItemValue, y, 1,
                         samples_disabled ? 170 : 174, (fxVolume + 4) / 8, 2,
                         10);
        JE_rectangle(VGAScreen, xMenuItemValue - 2, y - 2, xMenuItemValue + 96,
                     y + 11, 242);
        break;
    ...
    // Draw status text.
    JE_textShade(VGAScreen, xMenuItemName, 190,
                 menuItems[*selectedMenuItemIndex].description, 15, 4,
                 PART_SHADE);
    ...
    if (currentPicker != MENU_ITEM_NONE) {
      ...
      JE_rectangle(VGAScreen, xMenuItemValue - 5, yPicker - 3,
                   xMenuItemValue + wMenuItemValue + 5 - 1,
                   yPicker + hPicker + 3 - 1, 248);
      JE_rectangle(VGAScreen, xMenuItemValue - 4, yPicker - 4,
                   xMenuItemValue + wMenuItemValue + 4 - 1,
                   yPicker + hPicker + 4 - 1, 250);
      JE_rectangle(VGAScreen, xMenuItemValue - 3, yPicker - 5,
                   xMenuItemValue + wMenuItemValue + 3 - 1,
                   yPicker + hPicker + 5 - 1, 248);
      fill_rectangle_wh(VGAScreen, xMenuItemValue - 2, yPicker - 2,
                        wMenuItemValue + 2 + 2, hPicker + 2 + 2, 224);
      ...
      for (size_t i = 0; i < pickerItemsCount; ++i) {
        ...
        const char *value =
            selectedMenuItem->getPickerItem(i, buffer, sizeof buffer);

        draw_font_hv_shadow(VGAScreen, xMenuItemValue, y, value, normal_font,
                            left_aligned, 15, -3 + (selected ? 2 : 0), false,
                            2);
      }
    }
    ...
    if (currentPicker == MENU_ITEM_NONE) {
              ...
              if (newmouse && lastmouse_but == SDL_BUTTON_LEFT &&
                ...
                // Act on menu item via value.
                  ...
                  case MENU_ITEM_MUSIC_VOLUME: {
                    ...
                    int value = (lastmouse_x - xMenuItemValue) * 255 /
                                (wMenuItemValue - 1);
      ...
      if (action) {
        const MenuItemId selectedMenuItemId =
            menuItems[*selectedMenuItemIndex].id;
      ...
      if (mouseMoved || newmouse) {
        ...
        if (mouse_x >= xMenuItemValue &&
            mouse_x < xMenuItemValue + wMenuItemValue) {
          for (size_t i = 0; i < pickerItemsCount; ++i) {
            ...
            if (mouse_y >= yPickerItem && mouse_y < yPickerItem + hPickerItem) {
              ...
              if (newmouse && lastmouse_but == SDL_BUTTON_LEFT &&
                  ...
                  lastmouse_y < xMenuItemValue + wMenuItemName &&
                  lastmouse_y >= yPickerItem &&
                  lastmouse_y < yPickerItem + hPickerItem) {
                action = true;
              }
            }
          }
      ...
      if (newmouse) {
        ...
        case SDL_SCANCODE_UP: {
          ...
          const size_t pickerItemsCount =
              selectedMenuItem->getPickerItemsCount();
