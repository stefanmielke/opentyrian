...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x252A4, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC1F5E, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5C5B8, SEEK_SET);
#endif
...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x2662E, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC32E8, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5D942, SEEK_SET);
  if (episodeNum >= 4)
...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x26E21, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC3ADB, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5E135, SEEK_SET);
#endif
...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x26F24, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC3BDE, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5E238, SEEK_SET);
#endif
...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x2722F, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC3EE9, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5E543, SEEK_SET);
#endif
...
#ifdef TYRIAN2000
  if (episodeNum <= 3)
    fseek(f, 0x27EF3, SEEK_SET);
  if (episodeNum == 4)
    fseek(f, 0xC4BAD, SEEK_SET);
  if (episodeNum == 5)
    fseek(f, 0x5F207, SEEK_SET);
#endif
...
void JE_initEpisode(JE_byte newEpisode) {
  ...
  snprintf(levelFile, 13, "tyrian%d.lvl", episodeNum);
  snprintf(cube_file, 13, "cubetxt%d.dat", episodeNum);
  snprintf(episode_file, 13, "levels%d.dat", episodeNum);
